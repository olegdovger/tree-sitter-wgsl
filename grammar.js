...
module.exports = grammar({
  ...
  rules: {
    ...
    type_alias_declaration: ($) =>
      seq("alias", $.identifier, "=", $.type_declaration),
    ...
    function_return_type_declaration: ($) =>
      seq("->", repeat($.attribute), $.type_declaration),
    ...
    struct_member: ($) =>
      seq(repeat($.attribute), $.variable_identifier_declaration),
    ...
    _literal_or_identifier: ($) =>
      choice($.float_literal, $.int_literal, $.identifier),
    ...
    switch_statement: ($) =>
      seq("switch", $._expression, "{", repeat1($.switch_body), "}"),
    ...
    case_selectors: ($) =>
      seq($.const_literal, repeat(seq(",", $.const_literal)), optional(",")),

    case_compound_statement: ($) =>
      seq("{", repeat($._statement), optional($.fallthrough_statement), "}"),
    ...
    for_statement: ($) =>
      seq("for", "(", $.for_header, ")", $.compound_statement),
    ...
    while_statement: ($) =>
      seq("while", field("condition", $._expression), $.compound_statement),
    ...
    continuing_statement: ($) =>
      seq("continuing", $.continuing_compound_statement),

    continuing_compound_statement: ($) =>
      seq("{", repeat($._statement), optional($.break_if_statement), "}"),
    ...
    variable_qualifier: ($) =>
      seq("<", $.address_space, optional(seq(",", $.access_mode)), ">"),

    variable_identifier_declaration: ($) =>
      seq(field("name", $.identifier), ":", field("type", $.type_declaration)),
    ...
    const_literal: ($) =>
      choice($.int_literal, $.float_literal, $.bool_literal),
    ...
    type_declaration: ($) =>
      choice(
        ...
        "sampler",
        "sampler_comparison",
        ...["2d", "2d_array", "cube", "cube_array", "multisampled_2d"].map(
          (s) => "texture_depth_" + s,
        ),
        ...[
          "1d",
          ...
          .map((s) => "texture_" + s)
          .map((t) =>
            withTypeParameter($, t, choice("f16", "f32", "i32", "u32")),
          ),
        ...["1d", "2d", "2d_array", "3d"]
    ...
    _mat_prefix: ($) =>
      choice(
        ...cartesianProduct([2, 3, 4], [2, 3, 4]).map(
          ([n, m]) => `mat${n}x${m}`,
        ),
      ),
    ...
    texel_format: ($) =>
      choice(
        ...
        ...cartesianProduct(
          ["rgba16", "r32", "rg32", "rgba32"],
          ["uint", "sint", "float"],
        ).map(([t, s]) => t + s),
      ),

    address_space: ($) =>
      choice("function", "private", "workgroup", "uniform", "storage"),
    ...
    bitcast_expression: ($) =>
      seq("bitcast", "<", $.type_declaration, ">", $.parenthesized_expression),
    ...
    postfix_expression: ($) =>
      prec.left(
        PREC.UNARY,
        seq(
          // TODO consider replacing the subscript_expression and composite_value_decomposition_expression
          choice(
...
function cartesianProduct(...lists) {
  return lists.reduce((as, bs) =>
    as.flatMap((a) => bs.map((b) => [a, b].flat())),
  );
}
